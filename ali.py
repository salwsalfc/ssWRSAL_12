...
async def onerstart(event):
	sender = await event.get_sender()
	if sender.id == onerabbas_id:
		order = await event.reply('السورس يعمل بنجاح✓ ')
@abbas.on(events.NewMessage(outgoing=True,pattern='.فحص'))
async def ms(event):
		
		await event.reply(f'''السورس يعمل بنجاح قم بارسال ( .الاوامر ) ''')
...
async def _(event):
    ...
    if sender.id == onerabbas_id:
        ...
        await abbas.send_message(bot_username8, '/start')
        await asyncio.sleep(4)
        msg0 = await abbas.get_messages(bot_username8, limit=1)
        await msg0[0].click(1)
        await asyncio.sleep(4)
        msg1 = await abbas.get_messages(bot_username8, limit=1)
        await msg1[0].click(0)
        chs = 1
        for i in range(100):
            await asyncio.sleep(4)
            ...
            if msgs.message.find('لا يوجد قنوات حالياً 🤍') != -1:
                ...
                break
            msg_text = msgs.message  # الكود تمت كتابتهُ من قبل سورس الفراعنةممنوع الخمط
            if "اشترك فالقناة @" in msg_text:
                aljoker_channel = msg_text.split('@')[1].split()[0]
                try:
                    entity = await abbas.get_entity(aljoker_channel)
                    if entity:
                        await abbas(JoinChannelRequest(entity.id))
                        await asyncio.sleep(4)
                        msg2 = await abbas.get_messages(bot_username8, limit=1)
...
async def _(event):
    await event.reply("جاري تجميع النقاط")
    channel_entity = await abbas.get_entity(bot_username8)
    await abbas.send_message('@DamKombot', '/start')
    await asyncio.sleep(4)
    msg0 = await abbas.get_messages(bot_username8, limit=1)
    await msg0[0].click(1)
    await asyncio.sleep(4)
...
#                await abbas.send_message(event.chat_id, f"**خطأ , ممكن تبندت**")
                continue
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.تحويل دعمكم (.*) (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1)
    pt1= event.pattern_match.group(2) 
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
        send = await abbas.send_message(bot_username8, '/start')
        sleep(2)
        msg1 = await abbas.get_messages(bot_username8, limit=1)
        await msg1[0].click(4)
        sleep(4)
        await abbas.send_message(bot_username8, pt)
        sleep(4)
        await abbas.send_message(bot_username8, pt1)
        sleep(4)
        msg = await abbas.get_messages(bot_username8, limit=1)
        await msg[0].forward_to(onerabbas_id)
@abbas.on(events.NewMessage(outgoing=False, pattern='.كود (.*) '))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
        ...
        msg = await abbas.get_messages(bot_username8, limit=1)
        await msg[0].forward_to(onerabbas_id)                                                                        
@abbas.on(events.NewMessage(outgoing=False, pattern='.هدية دعمكم (.*)'))
async def OwnerStart(event):
        sender = await event.get_sender()
        if sender.id == onerabbas_id :
           await event.reply('جارٍ جمع الهدية اليومية ') 
           send = await abbas.send_message(bot_username8, '/start')
           sleep(2)
           msg1 = await abbas.get_messages(bot_username8, limit=1)
    ...
    if event.message.message.startswith("تمويل") and event.sender_id in DevShahm:
        ...
        if channel_username:
            try:
                ...
                response = "**𓀪︙ تم الانضمام إلى القناة بنجاح!**"
            except ValueError:
                response = "خطأ في العثور على القناة. يرجى التأكد من المعرف الصحيح"
        else:
            response = "**𓀪︙ يُرجى تحديد معرف القناة او المجموعة مع التمويل يامطوري ❤️** "
        await event.reply(response)
async def aljoker_nshr(abbas, sleeptimet, chat, message, seconds):
    global yaAli
    yaAli = True
    while yaAli:
        if message.media:
            sent_message = await abbas.send_file(chat, message.media, caption=message.text)
        else:
            sent_message = await abbas.send_message(chat, message.text)
        await asyncio.sleep(sleeptimet)
@abbas.on(events.NewMessage(outgoing=False, pattern=r"^\.نشر (\d+) (@?\S+)$"))
async def Hussein(event):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
	    abbas = event.client
	    joinu = await abbas(JoinChannelRequest('BGGlG'))
	    await event.delete()
	    ...
	    for chat_username in chat_usernames:
	        try:
	            chat = await abbas.get_entity(chat_username)
	            await aljoker_nshr(abbas, seconds, chat.id, message, seconds)  # تمرير قيمة seconds هنا لكل مجموعة
	        except Exception as e:
	            await event.reply(f"⌔∮ لا يمكن العثور على المجموعة أو الدردشة {chat_username}: {str(e)}"
	            )
async def aljoker_allnshr(event,abbas, sleeptimet, message):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
	    global yaAli
	    yaAli = True
	    aljoker_chats = await abbas.get_dialogs()
	    while yaAli:
	        for chat in aljoker_chats:
	            if chat.is_group:
	                try:
	                    if message.media:
	                        await abbas.send_file(chat.id, message.media, caption=message.text)
	                    else:
	                        await abbas.send_message(chat.id, message.text)
	                except Exception as e:
	                    print(f"Error in sending message to chat {chat.id}: {e}")
	        await asyncio.sleep(sleeptimet)        
@abbas.on(events.NewMessage(outgoing=False, pattern=r"^\.نشر_كروبات (\d+)$"))
async def Hussein(event):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
	    abbas = event.client
	    ...
	    global yaAli
	    yaAli = True
	    await aljoker_allnshr(event,abbas, sleeptimet, message)
super_groups = ["super", "سوبر"]
async def aljoker_supernshr(event,abbas, sleeptimet, message):
	    global yaAli
	    yaAli = True
	    aljoker_chats = await abbas.get_dialogs()
	    while yaAli:
...
@abbas.on(events.NewMessage(outgoing=False, pattern=r"^\.سوبر (\d+)$"))
async def Hussein(event):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
	    abbas = event.client
	    ...
	    global yaAli
	    yaAli = True
	    await aljoker_supernshr(event,abbas, sleeptimet, message)
    
@abbas.on(events.NewMessage(outgoing=False, pattern=r"^.اوامر النشر$"))
async def Hussein(event):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
    	joker_313 = await event.reply("""**
	𓀌 قـائمة اوامر النشر التلقائي للمجموعات
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.ايقاف النشر'))
async def stop_aljoker(event):
    sender = await event.get_sender()    
    if sender.id == onerabbas_id:
	    global yaAli
	    yaAli = False
	    await event.reply("**𓀪︙ تم ايقاف النشر التلقائي بنجاح ✓** ")
